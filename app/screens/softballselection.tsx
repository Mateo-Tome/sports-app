...
export default function SoftballSelection() {
  ...
  const go = (style: 'hitting' | 'pitching') => {
    router.push({
      ...
      params: { athlete, sport: 'softball', style },
    });
  };
  ...
  return (
    <>
      ...
      >
        <Text style={{ color: 'white', fontSize: 22, fontWeight: '900' }}>Choose softball mode</Text>
        <Text style={{ color: '#AAA', marginTop: 6, marginBottom: 10 }}>Recording — {athlete}</Text>

        <CardBtn
          icon="🥎"
          title="Hitting"
          subtitle="At-bats, contact, RBI & results"
          onPress={() => go('hitting')}
        />
        ...
        <CardBtn
          icon="💪"
          title="Pitching"
          subtitle="Bullpens, live pitching & outcomes"
          onPress={() => go('pitching')}
        />
      </View>
    </>
